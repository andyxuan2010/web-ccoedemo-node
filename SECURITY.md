...
- Use a random `SESSION_SECRET` of at least 32 characters in production. The
  application also enforces this requirement whenever it runs in Azure App
  Service, even if `NODE_ENV` is not explicitly set to `production`.
