...
- `.github/workflows/azure-webapp.yml` validates pull requests and pushes to `main`, uses least-privilege default permissions, runs the same quality gates, builds the ZIP artifact, creates release tags only from `main`, and optionally deploys and publishes mirrors. GitHub staging snapshots exclude `.github/` so workflows and Dependabot run only in the source repository.
