...
- `azure-pipelines.yml` builds `app.zip`, stages runtime files without `node_modules`, creates a semantic tag on successful non-PR runs, and deploys with `az webapp deploy` plus App Service build automation
- `.github/workflows/azure-webapp.yml` builds the same ZIP artifact, creates a semantic tag on successful non-PR runs, performs deployment prechecks, optionally deploys to App Service, and can publish to GitHub and Azure DevOps mirror repos
- `run_from_package.yml` builds a self-contained ZIP with `node_modules` and deploys with `WEBSITE_RUN_FROM_PACKAGE=1`

Current defaults:

- Node baseline is `24`
- Azure DevOps deploy stages target `web-platform-cc-sbx-node` for sandbox and `web-platform-eus-dev-node` for dev
- GitHub Actions targets `web-platform-eus-prod-node`, `web-platform-eus-dev-node`, and `web-platform-cc-sbx-node` by branch
