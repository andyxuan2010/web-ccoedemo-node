...
- `GET /healthz`: minimal unauthenticated health probe

## Deployment Files

Repo-shipped deployment automation:

- `azure-pipelines.yml` validates pull requests and pushes to `main`, runs lint/tests/dependency audit, builds `app.zip`, and deploys with App Service build automation
- `.github/workflows/azure-webapp.yml` validates pull requests and pushes to `main`, uses least-privilege default permissions, runs the same quality gates, builds the ZIP artifact, creates release tags only from `main`, and optionally deploys and publishes mirrors
...
- Production startup requires a `SESSION_SECRET` of at least 32 characters.
