...
This checklist verifies behavior for `web-platform-eastus-dev-node`.

## Validation Snapshot

Repository scan performed on `2026-04-27`.
...
- Access to the target Azure App Services used by the pipeline:
  - `web-platform-eastus-dev-node`
  - `web-nateams-dev-node-601`
  - `web-ccoedemo-dev-node-602`
...
- It auto-triggers from `main`.
- It performs explicit target-existence checks before deployment.
...
```bash
# Primary home page
curl -I https://web-platform-eastus-dev-node.azurewebsites.net/

# Secondary home page
curl -I https://web-nateams-dev-node-601.azurewebsites.net/

# Third home page
curl -I https://web-ccoedemo-dev-node-602.azurewebsites.net/

# Direct profile without auth should redirect
curl -I https://web-platform-eastus-dev-node.azurewebsites.net/profile/msal
```
...
az webapp config appsettings list \
  --name web-platform-eastus-dev-node \
...
az resource show \
  --resource-group <resource-group> \
  --name web-platform-eastus-dev-node/config/web \
