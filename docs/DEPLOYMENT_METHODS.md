...
8. Runs `DeploySandbox` from `Build`.
9. Runs `DeployDev` after `DeploySandbox`.
10. Uses `azure-pipelines/deploy-stage.yml` for shared deploy logic.
...
- The old third deployment target path is no longer part of the active deploy template.
