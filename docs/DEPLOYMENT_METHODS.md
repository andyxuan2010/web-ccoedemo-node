...
| --- | --- | --- | --- | --- | --- |
| `azure-pipelines.yml` | Yes | `main`, `dev`, and `sbx` pushes and PRs | Azure Pipelines runner before deploy | ZIP with production `node_modules` mounted read-only | Default Azure DevOps path |
| `.github/workflows/azure-webapp.yml` | Yes | `main` pushes, PRs, and optional manual dispatch | GitHub runner before deploy | ZIP with production `node_modules` | Default GitHub Actions path |
...
3. Runs on pushes and pull requests for `main`, `dev`, and `sbx`.
4. Installs Node `24.x`, runs `npm ci`, then runs lint, HTTP tests, and the production dependency audit.
5. Prunes development dependencies and uses `scripts/stage-package.js` to stage the self-contained runtime payload into `app.zip`.
...
7. Enables `WEBSITE_RUN_FROM_PACKAGE=1`.
8. Disables App Service/Oryx remote build because dependencies are already packaged.
...
- `DeployDev`
  ...
  - primary app: `web-platform-cc-dev-node`
- Shared default
  - secondary app: blank unless explicitly set through `webAppNameSecondary`

`main` runs the build and validation stage but has no deployment stage in this pipeline. Deployments are branch-gated: `sbx` deploys sandbox and `dev` deploys dev.

Operational characteristics:

- Self-contained artifact with production `node_modules`; App Service does not reinstall dependencies.
...
3. Prunes development dependencies and uses the same `scripts/stage-package.js` implementation as Azure DevOps to build a self-contained ZIP with production `node_modules`.
...
10. Configures the Node runtime, disables Oryx remote build because dependencies are already packaged, checks SCM/Kudu connectivity, and deploys with `az webapp deploy --type zip`.
...
- Produces a larger, self-contained artifact and does not install dependencies on App Service.
...
- Mirror repository variables: `STAGE_REPOSITORY` (`owner/repository`), `ADO_REPO_URL`
