...
2. Install Node 24
3. `npm ci`
4. Run the common lint, HTTP test, and production dependency-audit gate
...
2. `DeployDev` uses the shared deploy template with service connection `sc-platform-dev` and primary target `web-platform-eus-dev-node`.
...
6. If detection identifies Linux, set `linuxFxVersion=NODE|24-lts`, configure `cd /home/site/wwwroot && npm start`, enable App Service build automation, and deploy the ZIP package with `az webapp deploy`.
...
- Automatic CI and pull-request validation are limited to the protected `main` branch.
