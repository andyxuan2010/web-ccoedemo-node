...
```text
https://web-platform-cc-dev-node.azurewebsites.net/auth/callback?code=0.A...&state=ab12cd34ef56...
```
...
2. Install Node 24
3. `npm ci`
4. Run the common Node-version, lint, HTTP test, and production dependency-audit gate
5. Prune development dependencies and stage the self-contained runtime payload with `scripts/stage-package.js`
6. Archive the staged payload into `app.zip`
7. Publish artifact

Post-build pipeline stages:

1. `DeploySandbox` uses the shared deploy template with service connection `sc-platform-sbx` and primary target `web-platform-cc-sbx-node`.
2. `DeployDev` uses the shared deploy template with service connection `sc-platform-dev` and primary target `web-platform-cc-dev-node`.
...
5. If detection identifies Windows, set `WEBSITE_NODE_DEFAULT_VERSION=~24` and deploy the self-contained ZIP package in run-from-package mode.
6. If detection identifies Linux, set `linuxFxVersion=NODE|24-lts`, configure `cd /home/site/wwwroot && npm start`, and deploy the self-contained ZIP package in run-from-package mode.
...
- The application entrypoint remains `node app.js` through `npm start` from the explicit `/home/site/wwwroot` directory on Linux and `web.config`/iisnode on Windows.
- The application has been validated on a Windows App Service 32-bit worker process.
- App Service build automation is disabled because production dependencies are included in the artifact.

Deployment model notes:

- The build artifact is produced once and reused for all deployment targets.
- Azure DevOps CI and pull-request validation cover `main`, `dev`, and `sbx`; only `dev` and `sbx` have deployment stages. GitHub Actions covers `main` pushes and pull requests targeting `main`.
- Deployment occurs only after the common `npm run check` quality gate passes.
...
- Use a strong `SESSION_SECRET` of at least 32 characters in App Service
  settings. App Service startup fails closed when it is missing or too short.
...
- Structured application, request, and authentication lifecycle logs are written as JSON to standard output/error; connect App Service diagnostics to the preferred centralized log workspace and alerts.
- `/metrics` exposes process-local Prometheus counters for HTTP requests and MSAL authentication outcomes. Aggregate per-instance metrics in the monitoring platform when scaling out.
