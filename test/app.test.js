...
test("an existing session hides all sign-in actions", async () => {
  const principal = Buffer.from(
    JSON.stringify({
      claims: [{ typ: "name", val: "Demo User" }],
    }),
  ).toString("base64");
  const response = await request(app)
    .get("/")
  ...
  assert.match(response.text, /View Easy Auth profile/);
  assert.match(response.text, /Sign out \(Easy Auth\)/);
  assert.doesNotMatch(response.text, /mode-signin-button/);
});
...
test("metrics endpoint exposes operational counters without identity data", async () => {
  await request(app).get("/").expect(200);
  const response = await request(app).get("/metrics").expect(200);

  assert.match(response.headers["content-type"], /^text\/plain/);
  assert.match(response.text, /web_ccoedemo_http_requests_total/);
  assert.doesNotMatch(response.text, /SESSION_SECRET|AAD_CLIENT_SECRET/);
});
